...
def check_blacklist(plate_text: str) -> bool:
    """Kiểm tra biển số xe có nằm trong danh sách đen hay không."""
    if not plate_text or plate_text == -1:
        return False
    try:
        # Cần đảm bảo rằng tên cột trong DB là 'plate_number' như trong các hàm của bạn
        cursor.execute("SELECT 1 FROM blacklist_vehicles WHERE plate_number = %s", (plate_text,))
        return cursor.fetchone() is not None
    except Exception as e:
        print(f"[DB Error - Blacklist Check]: {e}")
        return False
...
async def add_blacklist_vehicle(request: Request):
    ...
    try:
        ...
        cursor.execute(
            ...
            "VALUES (%s, %s, %s, %s)",
            (plate_number, vehicle_type, description, 1)  # Thay 'admin' bằng ID người dùng nếu cần
        )
        db.commit()
